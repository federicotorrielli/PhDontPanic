...
def find_newest_pdf(download_dir, existing_files):
    ...
    if not new_pdfs:
        return None
    newest_pdf = max(new_pdfs, key=lambda f: os.path.getctime(os.path.join(download_dir, f)))
    return newest_pdf
...
print(f"All files processed! {len(no_doi)} entries have no DOI.\nEntries with no DOI: {no_doi}")